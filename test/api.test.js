...
test('OnlyOffice 集成默认关闭且启用后完成配置、签名内容访问与回调保存', async (t) => {
  ...
  // 浏览器配置令牌：配置字段必须在 payload 顶层（DS 依赖 decoded.document.key）
  ...
  assert.equal(configTokenPayload.document.fileType, 'docx');
  assert.match(configTokenPayload.editorConfig.callbackUrl, /\/callback\?token=/);
