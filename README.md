...
- OnlyOffice：启用时在 `.env` 中设置 `OFFICE_ENABLED=1 OFFICE_JWT_SECRET=<随机密钥>` 后执行 `docker compose up -d --build`（office 服务随 compose 一起启动；只想跑主应用用 `docker compose up -d cloudirve`），并设置 `OFFICE_URL=http://office`、`OFFICE_PUBLIC_URL=<浏览器可访问的 Document Server 地址>`、`OFFICE_CALLBACK_ORIGIN=<Document Server 可访问的 Cloudirve 地址>`。Document Server 默认限制 4GB 内存，NAS 内存不足时建议外置部署。
